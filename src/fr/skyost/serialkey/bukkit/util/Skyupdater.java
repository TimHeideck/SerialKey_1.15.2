...
public class Skyupdater {
	...
	public final String getLatestFileInfo(final InfoType type) {
		waitForThread();
		return updateData.get(type.getJSONKey()).getAsString();
	}
		...
		public void run() {
			try {
				...
				if(response != null && !response.equals("[]")) {
					final JsonArray jsonArray = jp.parse(response).getAsJsonArray();
					updateData = jsonArray.get(jsonArray.size() - 1).getAsJsonObject();
